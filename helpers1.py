...
def stats_grabber(crawled_feed):
    ...
    #find the highest & lowest sentiment score in content
    ...
    #same for title
    max_pos_t = crawled_feed.nlargest(1,'Title Score')
    max_pos_t['Content'] = textwrap.shorten(str(max_pos_t['Content']),width=100)
    max_neg_t = crawled_feed.nsmallest(1,'Title Score')
    max_neg_t['Content'] = textwrap.shorten(str(max_neg_t['Content']),width=100)

    # return all objects and pass them into the app.
    return crawled_feed, max_pos, max_neg, max_pos_t,max_neg_t, content_sentiment_outcome, title_sentiment_outcome


# function to display chart, if there is a category not represented, set it to 0 to avoid errors.
def sentiment_title_chart(outcome,title):
    ...
    try:
        ...
        neutral = 0
    # Almost similar as the CS50 version.
    figure = {
        "data": [
            {
...
def category_chart(dataframe,column,title):
    outcome = dataframe[column].value_counts()
    print([outcome])
    print([outcome.keys()])
    figure = {
        "data": [
            {
                "labels": [outcome.keys()[i] for i in range(len(outcome))],
                "hoverinfo": "none",
                "marker": {
                    "colors": [
                        "rgb(0,255,00)",
                ...
                "type": "pie",
                "values": [outcome[x] for x in range(len(outcome))]
            }
        ],
        "layout": {
            "title": title,
...
# A function for displaying sentiment via a box plot
...
# Improved version, when data has to be reshaped and then displayed as a box plot. Works for now only on one level.
# Deeper levels for now not necessary
...
# if I wanted to take a screenshot from the website.
# Unfortunately it will slow the app by a lot. and a lot of fiddling with diskpace size.
# Future iteration might include it.
